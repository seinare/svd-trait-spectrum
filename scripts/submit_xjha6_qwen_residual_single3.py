...
BASE = f"/data1/xjh/runs/svd-trait-spectrum/xjha6_qwen_residual_single3_{STAMP}"
...
mkdir -p "$RUN_ROOT"/{{logs,state,tmp,monitor,lm_eval,standard_cot}} "$TMPDIR"
...
run_once() {{
  ...
  case "$stage" in
    standard_cot)
      local common=(--alpha "$alpha" --model_id "$MODEL_ID" --tensor_parallel_size 1 --gpu_memory_utilization 0.50 --enforce_eager --tmp_root "$RUN_ROOT/tmp/alpha_${{alpha}}" --max_model_len 8192 --max_tokens 8192 --output_root "$RUN_ROOT/standard_cot")
      ...
      ;;
    lm_eval)
      local common=(--alpha "$alpha" --model_id "$MODEL_ID" --tensor_parallel_size 1 --gpu_memory_utilization 0.45 --enforce_eager --tmp_root "$RUN_ROOT/tmp/alpha_${{alpha}}" --max_model_len 4096 --backend vllm --preset requested --batch_size 1 --force --output_root "$RUN_ROOT/lm_eval")
      if [ -n "$prep" ]; then common+=(--prepared_model_dir "$prep"); fi
