...
BASE = f"/data1/xjh/runs/svd-trait-spectrum/xjha6_sbj9_{STAMP}"
...
mkdir -p "$RUN_ROOT"/{{logs,state,tmp,monitor,standard,bfcl,judge}} "$MODEL_ROOT" "$TMPDIR"
...
monitor() {{
  while true; do
    {{
      ...
      ps -eo pid,ppid,stat,etime,pcpu,pmem,cmd | grep -E "eval_script|vllm|uv run" | grep -v grep || true
...
run_once() {{
  ...
  common=(--alpha "$alpha" --model_id "$MODEL_ID" --tensor_parallel_size 1 --gpu_memory_utilization 0.72 --max_model_len 2048 --enforce_eager --tmp_root "$RUN_ROOT/tmp/alpha_${{alpha}}")
  ...
  case "$stage" in
    standard)
      if [ -n "$prep" ] && [ -d "$prep" ]; then
        uv run python scripts/eval_script1_standard.py "${{common[@]}}" --max_tokens 256 --prepared_model_dir "$prep" --output_root "$RUN_ROOT/standard" > "${{log}}.out" 2> "${{log}}.err"
      elif [ -n "$prep" ]; then
        uv run python scripts/eval_script1_standard.py "${{common[@]}}" --max_tokens 256 --save_prepared_model_dir "$prep" --output_root "$RUN_ROOT/standard" > "${{log}}.out" 2> "${{log}}.err"
      else
        uv run python scripts/eval_script1_standard.py "${{common[@]}}" --max_tokens 256 --output_root "$RUN_ROOT/standard" > "${{log}}.out" 2> "${{log}}.err"
      fi
      ;;
    bfcl)
...
for alpha in "${{ALPHAS[@]}}"; do
  for stage in standard bfcl judge; do
    run_stage "$alpha" "$stage" || {{
      overall_rc=$?
