...
BASE = f"/data1/xjh/runs/pruning_codex/llama1b_6alpha_5modules_{STAMP}"
...
prepared_dir() {{
  ...
  if [ "$alpha" = "0" ] || [ "$alpha" = "0.0" ]; then
    ...
    echo "$MODEL_ROOT/Llama-3.2-1B-Instruct-alpha${{alpha}}"
  fi
}}
...
run_once() {{
  ...
  common=(--alpha "$alpha" --model_id "$MODEL_ID" --local_files_only --tensor_parallel_size 1 --gpu_memory_utilization 0.5 --enforce_eager --tmp_root "$RUN_ROOT/tmp/alpha_${{alpha}}")
