...
BASE = f"/data1/xjh/runs/pruning_codex/llama31_8b_7alpha_5modules_{STAMP}"
CODE = "/data1/xjh/code/pruning_codex"
MODEL_ID = "/data1/xjh/code/pruning/models/Llama-3.1-8B-Instruct"
MODEL_ROOT = "/data1/xjh/models/pruning_codex/llama31_8b"
...
prepared_dir() {{
  ...
  if [ "$alpha" = "0" ] || [ "$alpha" = "0.0" ]; then
    ...
    echo "$MODEL_ROOT/Llama-3.1-8B-Instruct-alpha${{alpha}}"
  fi
}}
...
run_once() {{
  ...
  common=(--alpha "$alpha" --model_id "$MODEL_ID" --local_files_only --tensor_parallel_size 1 --gpu_memory_utilization 0.54 --enforce_eager --tmp_root "$RUN_ROOT/tmp/alpha_${{alpha}}")
  ...
  case "$stage" in
    ...
    bfcl)
      ...
      ;;
    judge)
      uv run python scripts/eval_script3_judge.py "${{common[@]}}" --max_model_len 4096 --judge_model v4-flash --judge_workers 8 --output_root "$RUN_ROOT/judge" > "${{log}}.out" 2> "${{log}}.err"
      ;;
    lm_eval)
      uv run python scripts/eval_script4_lm_eval_tasks.py "${{common[@]}}" --max_model_len 4096 --backend vllm --preset requested --batch_size 1 --force --output_root "$RUN_ROOT/lm_eval" > "${{log}}.out" 2> "${{log}}.err"
      ;;
    standard_cot)
